...
<table id="mystudentsprofile-table" class="table table-striped">
...
$sub_domain=$sub_domaincon->dom();
  ...
  const subdominio1 = `'.$sub_domain.'/lugmacore/apiResources/v1/allMyStudentsProfile/${my_profyle}`;


 // Función para obtener los datos del API
 async function getStudentsProfile() {
	
	fetch(subdominio1)
  .then(response => response.json())
  .then(data => {
    const publicgroupsTableBody = document.querySelector("#mystudentsprofile-table tbody");
