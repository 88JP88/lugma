...
<table id="mycoors-table" class="table table-striped">
...
$sub_domain=$sub_domaincon->dom();
  ...
  const subdominiocoors = `'.$sub_domain.'/lugmacore/apiResources/v1/allMyCoor/${my_profyle}`;


 // Función para obtener los datos del API
 async function getCoors() {
	
	fetch(subdominiocoors)
  .then(response => response.json())
  .then(data => {
    const publicgroupsTableBody = document.querySelector("#mycoors-table tbody");
