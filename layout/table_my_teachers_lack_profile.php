...
<table id="myteacherslackprofile-table" class="table table-striped">
...
$sub_domain=$sub_domaincon->dom();
  ...
  const subdominioteacherslprofile = `'.$sub_domain.'/lugmacore/apiResources/v1/allMyTeachersLackProfile/${my_profyle}`;


 // Función para obtener los datos del API
 async function getTeachersLackProfile() {
	
	fetch(subdominioteacherslprofile)
  .then(response => response.json())
  .then(data => {
    const publicgroupsTableBody = document.querySelector("#myteacherslackprofile-table tbody");
    ...
    data.teachers.forEach(student => {
      ...
      row.innerHTML = `
      <td>
      <a class="btn btn-primary" href="controller/controller_post_teachers_post.php?teacher_id=${student.teacher_id}&s_name=${student.name}&s_lname=${student.last_name}&my_profile=${student.profile_id}&owner_id=${student.owner_id}">Crear Perfil</a>
