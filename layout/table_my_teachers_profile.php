...
<table id="myteachersprofile-table" class="table table-striped">
...
$sub_domain=$sub_domaincon->dom();
  ...
  const subdominioteachersprofile = `'.$sub_domain.'/lugmacore/apiResources/v1/allMyTeachersProfile/${my_profyle}`;


 // Función para obtener los datos del API
 async function getTeachersProfile() {
	
	fetch(subdominioteachersprofile)
  .then(response => response.json())
  .then(data => {
    const publicgroupsTableBody = document.querySelector("#myteachersprofile-table tbody");
    ...
    data.teachers.forEach(student => {
      ...
      <a class="btn btn-primary" href="group.php?teacher_id=${student.teacher_id}&my_profile=${student.profile_id}&owner_id=${student.owner_id}">Abrir</a>  </td>
        <td>${student.name}</td>
