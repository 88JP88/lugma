...
<table id="mycoorsprofile-table" class="table table-striped">
...
$sub_domain=$sub_domaincon->dom();
  ...
  const subdominio1coors = `'.$sub_domain.'/lugmacore/apiResources/v1/allMyCoorProfile/${my_profyle}`;


 // Función para obtener los datos del API
 async function getCoorsProfile() {
	
	fetch(subdominio1coors)
  .then(response => response.json())
  .then(data => {
    const publicgroupsTableBody = document.querySelector("#mycoorsprofile-table tbody");
