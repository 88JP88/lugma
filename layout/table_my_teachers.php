...
<table id="myteachers-table" class="table table-striped">
...
$sub_domain=$sub_domaincon->dom();
  ...
  const subdominioteachers = `'.$sub_domain.'/lugmacore/apiResources/v1/allMyTeachers/${my_profyle}`;


 // Función para obtener los datos del API
 async function getTeachers() {
	
	fetch(subdominioteachers)
  .then(response => response.json())
  .then(data => {
    const publicgroupsTableBody = document.querySelector("#myteachers-table tbody");
